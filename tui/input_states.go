...
func (m Model) updateFileInput(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		if m.fileInput != "" {
			...
			m.logFile = "panos.xml" // Use default when field is empty
		}
...
func (m Model) viewFileInput() string {
	...
	if m.fileInput == "" {
		...
		s.WriteString(inputFieldStyle.Render("File: ") + placeholderStyle.Render("panos.xml") + cursor + "\n\n")
	} else {
...
func (m Model) updateDeviceGroupInput(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		return m.startDeviceGroupDiscovery()
	case "backspace":
...
func (m Model) updateDeviceGroupSelection(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter", " ":
		if m.cursor < len(m.discoveredDeviceGroups) {
			...
			return m.startDeviceGroupDuplicateScan()
		}
	}
...
// Missing methods - simplified implementations for XML architecture

// startDeviceGroupDiscovery - simplified implementation
func (m Model) startDeviceGroupDiscovery() (Model, tea.Cmd) {
	return m, discoverDeviceGroupsCmd(m.logFile)
}

// startDeviceGroupDuplicateScan - simplified implementation
func (m Model) startDeviceGroupDuplicateScan() (Model, tea.Cmd) {
	m.state = StateProcessing
	return m, processFileCmd(m.logFile, []string{})
}

// generateAllAddressGroupCommandsWithMappings - simplified implementation
